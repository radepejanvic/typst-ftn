...
//     ),
//     caption: [
//         Типови хипервизора: лево – тип 1 (енгл. _native_, _bare-metal_); десно – тип 2 (енгл. _hosted_).
//     ]
// )<fig:hipervizori>
...
caption: [
    Хипервизор тип 1 (енгл. _native_, _bare-metal_).
]
)<fig:hipervizor-1>

#figure(image("../slike/hipervizor-tip-2.svg", width: 50%),
caption: [
    Хипервизор тип 2 (енгл. _hosted_).
]
)<fig:hipervizor-2>
...
Главна предност контејнеризације у односу на класичну виртуелизацију јесте њена ефикасност. Будући да се више контејнера може покретати на једном оперативном систему без потребе за додатним хипервизором, трошак меморије и процесорских ресурса је знатно мањи, а време покретања апликација драстично краће. Сваки контејнер се извршава као посебан процес у корисничком простору (енгл. _user space_), док све виртуелне машине захтевају покретање комплетног оперативног система, што их чини знатно „тежим“ и споријим. Овакав приступ омогућава већу густину апликација по систему и једноставније оркестрирање @docker_what_container. 
...
У склопу алата имплементиран је један позадински модул који користи _Vagrant_ @hashicorp_vagrant и _go-vagrant_ @matcuk_go_vagrant библиотеку за подизање и управљање животним циклусом чворова.  Конфигурациони фајл је дефинисан у YAML формату, док су скрипте за провизионисање (енгл. _provisioning_) и покретање сервиса реализоване као _Shell_ скрипте. _VirtualBox_ @virtualbox_homepage хипервизор је коришћен као провајдер виртуелизације. _Control plane_ и сервиси покретани на виртелним машинама су спаковани у _Docker_ @docker_homepage контејнере и њихово покретање је вршено помоћу _Docker Compose_ @docker_compose_docs алата.   
...
_Docker_ је платформа за контејнеризацију. Oмогућава паковање апликације и свих њених зависности у преносиви контејнер. Контејнери се извршавају из _Docker_ слика (енгл. _images_), које представљају шаблоне са свим неопходним зависностима, оперативним окружењем, библиотекама и конфигурацијама за покретање апликације. _Docker_ слике се дефинишу помоћу _Dockerfile_ датотека. 
_Docker_ такође подржава употребу дељених директоријума (енгл. _volumes_) за трајно складиштење података изван животног циклуса контејнера.
