...
Имплементација је реализована у програмском језику Go, који омогућава компајлирање у једну извршну датотеку. _Backend_ је заснован на _Vagrant_-у и _VirtualBox_-у, док се параметри окружења дефинишу путем YAML конфигурационих датотека.
...
У табели #link(<tbl:proj>
)[3] приказана је структура пројекта и намена сваког директоријума. Табела пружа јасан преглед организације изворног кода и служи као оријентир за даље разумевање имплементације система.
...
table(
  ...
  [`/internal/backends`], [Имплементације конкретних _backend_-а. Тренутно постоји само _VagrantBackend_.],
...
== Конфигурација
<конфигурација>
Конфигурациона датотека у YAML формату дефинише параметре окружења, као што су број виртуелних машина, дистрибуција оперативног система, ресурси и мрежне поставке. На овај начин се омогућава лако прилагођавање и репродукција развојног окружења. Опис структуре конфигурационе датотеке дат је у табели #link(<tbl:yaml>
