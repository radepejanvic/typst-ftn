...
_Core_ слој чини централни део система и дефинише скуп функционалности (_interface_) које сваки _Backend_ мора да имплементира. Он управља животним циклусом виртуелних машина, оркестрира операције над њима и обезбеђује апстракцију између CLI и _Backend_ слојева.
...
Процес иницијализације окружења представља полазну тачку рада _Wormhole_ алата. Сврха ове функционалности је да на основу конфигурационe датотеке (табела #link(<tbl:yaml>)[2]) припреми све потребне параметре за креирање виртуелних машина и мрежно повезивање. Резултат овог корака је окружење спремно за покретање.

Корисник прво уноси команду `wormhole setup --config config.yml` у командну линију. CLI слој учитава конфигурациону датотеку на прослеђеној путањи и позива функцију за иницијализацију окружења из _Core_ слоја. _Core_ мапира позив на одговарајући _Backend_ (_VagrantBackend_ у овом случају). _VagrantBackend_ генерише _Vagrantfile_ на основу прослеђене конфигурације (слика #link(<fig:seq_diagram_1>)[8]). 
...
Успешним извршавањем ове функционалности, окружење добија све дефинисане виртуелне машине са инсталираним сервисима и зависностима, спремним за даље покретање (слика #link(<fig:seq_diagram_2>)[9]).
...
Корисник покреће команду `wormhole stop [node ...]` са опционим аргументом који одређује једну или више виртуелних машина за заустављање. Уколико аргумент није наведен, _Wormhole_ CLI аутоматски зауставља све машине дефинисане у конфигурацији. CLI слој прослеђује захтев _Core_ модулу, који позива методу из _VagrantBackend_-а задужен за суспензију виртуелних машина путем _Vagrant_ API-ја (слика #link(<fig:seq_diagram_3>)[10]). 
...
Корисник покреће команду `wormhole resume [node ...]` са опционим аргументом који одређује једну или више виртуелних машина које треба поново покренути. Уколико аргумент није наведен, Wormhole CLI аутоматски наставља рад свих виртуелних машина дефинисаних у конфигурацији. CLI слој прослеђује захтев _Core_ модулу, који затим позива методу из _VagrantBackend_-а задужену за наставак рада виртуелних машина путем _Vagrant_ API-ја (слика #link(<fig:seq_diagram_4>)[11]).
...
Корисник покреће команду `wormhole reload [node ...]` са опционим аргументом који одређује једну или више виртуелних машина које треба поново креирати и покренути. Уколико аргумент није наведен, _Wormhole_ CLI извршава поступак над свим машинама дефинисаним у конфигурацији. CLI слој прослеђује захтев _Core_ модулу, који затим позива методу из _VagrantBackend_-а задужену за поновно креирање и покретање виртуелних машина путем _Vagrant_ API-ја (слика #link(<fig:seq_diagram_5>)[12]).
...
Корисник покреће команду `wormhole shutdown [node ...]` са опционим аргументом који одређује једну или више виртуелних машина које треба искључити. Уколико аргумент није наведен, Wormhole CLI извршава поступак над свим машинама дефинисаним у конфигурацији. CLI слој прослеђује захтев _Core_ модулу, који затим позива методу из _VagrantBackend_-а задужену за гашење виртуелних машина путем _Vagrant_ API-ја (слика #link(<fig:seq_diagram_6>)[13]).
...
Корисник покреће команду `wormhole destroy [node ...]` са опционим аргументом који одређује једну или више виртуелних машина које треба поново креирати и покренути. Уколико аргумент није наведен, _Wormhole_ CLI извршава поступак над свим машинама дефинисаним у конфигурацији. CLI слој прослеђује захтев Core модулу, који затим позива методу из _VagrantBackend_-а задужену за уништавање виртуелних машина и уклањање њихових придружених ресурса путем _Vagrant_ API-ја (слика #link(<fig:seq_diagram_7>)[14]).
